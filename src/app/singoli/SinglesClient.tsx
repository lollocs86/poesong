...
function TrackRow({ track, index, isCurrentTrack, isPlaying, onPlay, onShare, playCount }: {
  ...
  onPlay: () => void; onShare: () => void; playCount: number;
}) {
  const [showMobileMenu, setShowMobileMenu] = useState(false);

  const handleShare = (e: React.MouseEvent) => { e.stopPropagation(); onShare(); setShowMobileMenu(false); };
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-1 flex-shrink-0">
        <button onClick={handleDownload} className="p-2 text-white/40 hover:text-white hover:bg-white/10 rounded-full transition-colors" title="Scarica MP3">
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>
        </button>
        <button onClick={handleShare} className="p-2 text-white/40 hover:text-white hover:bg-white/10 rounded-full transition-colors" title="Condividi">
        ...
        {showMobileMenu && (
          <>
            ...
            <div className="absolute right-0 top-full mt-1 z-50 bg-gray-800 rounded-lg shadow-xl border border-white/10 py-2 min-w-[160px]">
              <button onClick={handleDownload} className="w-full flex items-center gap-3 px-4 py-2 text-white/80 hover:bg-white/10 transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>
                Scarica MP3
              </button>
              <button onClick={handleShare} className="w-full flex items-center gap-3 px-4 py-2 text-white/80 hover:bg-white/10 transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92-1.31-2.92-2.92-2.92z"/></svg>
                Condividi
              </button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 md:p-8" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className="w-20 h-20 bg-gradient-to-br from-purple-600 to-pink-500 rounded-lg flex items-center justify-center flex-shrink-0">
            {track.coverUrl
              // eslint-disable-next-line @next/next/no-img-element
